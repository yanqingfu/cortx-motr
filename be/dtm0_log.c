...
M0_INTERNAL void
m0_be_dtm0_log_post_persistent(struct m0_be_dtm0_log        *log,
			       const struct m0_dtm0_tx_desc *txd)
{
	struct m0_dtm0_log_rec *rec;
	bool                    is_stable;
	...
	M0_PRE(log != NULL);
	/* TODO: assert that the log is volatile */
	M0_PRE(txd != NULL);
	M0_PRE(m0_dtm0_tx_desc__invariant(txd));
	...
	M0_ASSERT_INFO(rec != NULL, "Log record must be inserted when the dtx"
		       "is getting closed");
	 ...
	 */
	if (!is_stable)
		m0_dtm0_dtx_post_persistent(&rec->dlr_dtx, txd);
